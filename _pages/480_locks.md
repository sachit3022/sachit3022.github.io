...
class Connection(object):
    ...
    def __init__(self, filename):
        ...
        if filename in _ALL_DATABASES:
            self._database = _ALL_DATABASES[filename]
        else:
    ...
    def execute(self, statement):
        ...
        def create_table(tokens):
            ...
            """
            self.database.aquire_lock("exclusive")
        ...
        def select(tokens):
            ...
            self.database.aquire_lock("shared")
        ...
        def begin(tokens):
            #works on deferred state.
            state = "DEFERRED"
            if tokens[1] in set(["DEFERRED","EXCLUSIVE","IMMEDIATE"]):
                state = tokens[1]
            ...
            if state == "DEFERRED":
                self._database.aquire_lock("none")
            elif state == "EXCLUSIVE":
                self.database = self._database.make_private_copy()
                self.db_copy = True
                self.database.aquire_lock("reserved")
                
            elif state == "IMMEDIATE":
                self.database = self._database.make_private_copy()
                self.db_copy = True
                self.database.aquire_lock("exclusive")
                
                
        def commit(tokens=None):
            if tokens is None:
                self.database.release_locks()
            else:
                
                if self.database.exclusive_lock or self.database.reserved_lock:
                    self.database.aquire_lock("reserved")
                else:
                    self.database.aquire_lock("none")

                self.transaction_in_progess = False
                self._database.commit(self.database)
                self.database.release_locks()
                self.database = self._database
                self.db_copy = False
        ...
        if self.transaction_in_progess and self.db_copy !=True  and tokens[0] in set(["CREATE","INSERT","UPDATE","DELETE","SELECT","COMMIT"]):
            self.database = self.database.make_private_copy()
            self.db_copy = True

        if tokens[0] == "CREATE":
            create_table(tokens)
            if not self.transaction_in_progess:
                commit()
            return []
...
class Database:
    ...
    def __init__(self, filename):
        ...
        self.tables = {}
        self.shared_lock = False
        self.exclusive_lock = False
        self.reserved_lock = False
    ...
    def aquire_lock(self,lock = "shared"): #"shared", "exclusive", "reserved", "none"
        
        if lock == "shared":
            #write shared lock conditions later.
            if not self.shared_lock:
                Database._shared_lock += 1
                self.shared_lock = True
           
        elif lock == "exclusive":            
            if self.exclusive_lock == False and Database._exclusive_lock == 1:
                #some other database holds exclusive lock and only one exclusive lock is possible.
                raise Exception('Database is locked')
            else:
                Database._exclusive_lock = 1
                self.exclusive_lock = True


        elif lock == "reserved":
            other_databases_hold_shared_lock  = (Database._shared_lock ==1 and not self.shared_lock) or (Database._shared_lock >1)
            other_databases_hold_reserve_lock = Database._reserved_lock ==1  and not self.reserved_lock
            other_databases_hold_exclusive_lock =  Database._exclusive_lock ==1  and not self.exclusive_lock 
            if other_databases_hold_exclusive_lock or other_databases_hold_shared_lock or other_databases_hold_reserve_lock:
                raise Exception('Database is locked')
            
            Database._reserved_lock = 1 
            self.reserved_lock = True

        return True
    def make_private_copy(self):
        db_copy  = deepcopy(self)
        #initially doesnot hold nay locks
        db_copy.shared_lock = False
        db_copy.exclusive_lock = False
        db_copy.reserved_lock = False
        
        return db_copy
    
    def commit(self,databse_copy):
        self.tables = databse_copy.tables
    def release_locks(self):
        if self.exclusive_lock:
            Database._exclusive_lock = 0
            self.exclusive_lock = False
        if self.shared_lock:
            Database._shared_lock -= 1
            self.shared_lock = False
        if self.reserved_lock:
            Database._reserved_lock = 0
            self.reserved_lock = False
